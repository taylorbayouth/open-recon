...
const { navigate, ensureInView, clickablePoint, assertHittable, isTextInput } = require('./page');
...
async function click({ session, brief, ref }) {
  ...
  // visible viewport — not the raw bbox center, which can land on a
  ...
  // viewport-relative point (Input.dispatchMouseEvent's coordinate space),
  // falling back to the bbox center when quads are unavailable.
  const { x, y } = await clickablePoint({ session, brief, ref });
  // Verify the aim point isn't covered by an overlay before pressing; throws a
  // non-fatal error (→ error Observation, loop re-plans) if it is. Fails open.
  await assertHittable({ session, brief, ref, x, y });
...
async function selectText({ session, brief, ref, save }) {
  ...
  return { selectedText: await readSelection(client), save: save === true };
}
