...
const fs = require('fs');
const os = require('os');
const { ensureInView, navigate, clickablePoint, assertHittable, isTextInput } = require('./page');
...
// click and selectText target text (@t) or element (@e) refs, so look in both.
...
async function frontmostApp(client) {
  try {
    return await client.send({ op: 'frontapp' });
  } catch (err) {
...
// helper can't report it (e.g. an older recon-input binary without the `idle`
...
async function ensureInputSafe(client, opts = {}) {
  const verbose = opts.verbose === true || client.verbose === true;
  ...
  for (;;) {
    ...
    if (announcedFront && verbose) console.error('[os] Chrome is frontmost again; resuming.');
    ...
    if (idle.enabled) {
      ...
      if (announcedIdle && verbose) console.error('[os] user idle again; resuming.');
      announcedIdle = false;
    }

    // Both gates clear. Raise the current target's window to the front. CGEvents
    // land on whatever Chrome window is topmost at the screen coordinates we're
    // about to hit, so a popup or new tab we followed (see Session.followActiveTab)
    // must be brought forward or the input could strike the window behind it.
    // Best-effort: a failure (no Page domain, detached target) falls through to
    // the dispatch unchanged — same as before this existed.
    try { await client.Page.bringToFront(); } catch {}
    return; // both gates clear — safe to send input
  }
}
...
async function pageToScreen(session, pageX, pageY, opts = {}) {
  ...
  // callers (selectText, scroll) keep it.
  const localX = opts.viewportRelative ? pageX : (pageX - scrollX);
  const localY = opts.viewportRelative ? pageY : (pageY - scrollY);

  const screen = {
    x: windowBounds.left + chromeOffsetX + localX,
    y: windowBounds.top  + chromeOffsetY + localY,
  };
...
async function click({ session, brief, ref, client, humanize }) {
  ...
  // Aim at the element's real hit-region (DOM.getContentQuads), clamped to the
  // visible viewport, instead of the raw bbox center — which can land on a
  // non-interactive child of a large container or in the gap between a
  // multi-line link's lines. clickablePoint scrolls the target in and returns a
  // viewport-relative point (falling back to the bbox center if quads are
  // unavailable), so the prior behavior is preserved when the precise path can't run.
  ...
  // signal), but small enough to stay inside the hit-region.
  ...
  // Verify nothing is covering the aim point before we move/press. If an overlay
  // (modal, cookie wall, sticky bar) intercepts it, this throws a non-fatal error
  // so the loop can dismiss it rather than click blind. Fails open if it can't tell.
  await assertHittable({ session, brief, ref, x: aimX, y: aimY });

  const screen = await pageToScreen(session, aimX, aimY, { viewportRelative: true });
...
//
// Exception: any node (@t text OR @e element) that has an accessible name
// already carries its full text in node.name from the AX tree, so return it
// directly and skip the drag. This avoids three failure modes of the physical
// drag: (1) the visible cursor glitch, (2) empty selections on sites using
// user-select:none, and (3) — for draggable @e targets like links and images —
// triggering Chrome's native drag-and-drop, which yanks the link/image (or a
// browser tab) across the screen. The physical drag is reserved for nameless
// nodes (bare containers) where the AX tree has no text to hand back.
async function selectText({ session, brief, ref, client, humanize, save }) {
  const node = nodeByRef(brief, ref);
  if (!node) throw new Error(`node ${ref} not found in brief`);

  if (typeof node.name === 'string' && node.name.trim()) {
    return { selectedText: node.name, save: save === true };
  }
  ...
  return { selectedText: await readSelection(session.client), save: save === true };
}

// Read the current selection so the action can report what it highlighted. The
// one place we evaluate JS in the page — an action-time confirmation read, not
// part of perception, so the no-footprint extraction path is unaffected.
async function readSelection(cdpClient) {
  try {
...
async function type({ session, brief, ref, text, client, humanize, clear }) {
  ...
  // recon-input binary without letter keycodes rejects "a"; we swallow that and
  // fall back to appending rather than failing the whole type.
  if (clear !== false && isTextInput(nodeByRef(brief, ref))) {
    try {
      await client.send({ op: 'key', key: 'a', modifiers: ['cmd'] });
    } catch (err) {
      if (client.verbose) console.error(`[os] clear-before-type skipped: ${err.message}`);
    }
  }
...
function create(opts = {}) {
  ...
  client.verbose = opts.verbose === true;
  ...
  return {
    ...
    // Backend-agnostic — read/capture via session.client (CDP), not recon-input.
...
module.exports = { create, DEFAULT_HUMANIZE, ensureInputSafe };
