...
function describeAction(action, brief) {
  ...
  switch (action.verb) {
    ...
    case 'wait':   return `waited ${action.args?.ms ?? ''}ms`.trimEnd();
...
function termDesc(action, brief, observation) {
  ...
  switch (action.verb) {
    ...
    case 'scroll':          return `Scrolled ${action.args?.direction || 'down'}`;
    case 'press':           return `Pressed ${action.args?.key || ''}`;
    case 'wait':            return `⏱ Waited ${action.args?.ms ?? ''}ms`;
