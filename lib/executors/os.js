...
const PLATFORM_BIN = {
  darwin: path.resolve(__dirname, '..', '..', 'native', 'macos',  'recon-input', 'bin', 'recon-input'),
  linux:  path.resolve(__dirname, '..', '..', 'native', 'linux',  'recon-input', 'bin', 'recon-input'),
};
  ...
  async init() {
    ...
    if (!fs.existsSync(this.binPath)) {
      const buildScript = process.platform === 'linux'
        ? 'native/linux/recon-input/build.sh'
        : 'native/macos/recon-input/build.sh';
      throw new Error(
...
async function pageToScreen(session, pageX, pageY, opts = {}) {
  ...
  // On macOS, CSS-px and screen points are 1:1 (Quartz handles Retina below
  // this layer). On Linux, XTestFakeMotionEvent takes physical pixels, so we
  // must multiply by devicePixelRatio. Fetch it once per pageToScreen call —
  // it's cheap and handles mid-session display changes without caching issues.
  let dpr = 1;
  ...
  const screen = {
    x: (windowBounds.left + chromeOffsetX + localX) * dpr,
    y: (windowBounds.top  + chromeOffsetY + localY) * dpr,
  };
...
async function selectText({ session, brief, ref, client, humanize }) {
  ...
  await ensureInputSafe(client);
  // try/finally: the drag holds the left button down between the down and up. If
  // the extending move (or anything mid-drag) throws, we MUST still release it —
  // a stuck leftButtonDown turns every later click into an unintended drag.
  try {
    await client.send({ op: 'down', button: 'left' });
    await client.send({ op: 'move', x: end.x, y: end.y, speedPxPerSec: speed, jitterPx: jitter });
  } finally {
    await client.send({ op: 'up', button: 'left' }).catch(() => {});
  }
  return { selectedText: await readSelection(session.client) };
}
