...
function scrollLoopWarning(step, state, loopCfg) {
  const threshold = loopCfg.maxSameDirectionScrolls ?? loopCfg.maxSameIntentScrolls ?? 0;
  ...
  const direction = normalizeScrollDirection(step.action);
  const key = `${cleanUrl(step.url, { max: 0 }) || ''}|${direction}`;
  if (state.key === key) {
    state.count++;
  } else {
    state.key = key;
  ...
  if (state.count >= threshold && !state.warned) {
    state.warned = true;
    return `WARNING: scroll ${direction} repeated ${state.count}x on this page; pivot, save findings, go back, or finish.`;
  }
  return null;
}
...
async function run({ session, task, config = {} } = {}) {
  ...
  emit(`models · planner ${provider}:${plannerModel} · vision ${visionProvider}:${visionModel}`);
  ...
  const scrollLoopState = { key: null, count: 0, warned: false };

  // "Moment of silence" reflection state (see maybeReflect + lib/reflect.js).
  ...
  async function maybeReflect(reason, brief) {
    ...
    try {
      const { text, completion } = await reflect({
        task, url: brief?.url, title: brief?.title, saved, provider, model,
      });
    ...
    const line = `🧭 moment of silence (${reason}) → ${decision}`;
    events.push(line);
    emit(`\n${line}`);
  ...
  // clean exit would (report + latest.json) before exiting. finalize/finish use
  // synchronous writes, so they complete before process.exit. `aborting` guards a
  ...
  const onSigint = () => {
    ...
    try { finish(runArtifact, { events, scratchpad }); } catch {}
  ...
  try {
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      if (!budgetReflected && iter >= budgetTurn) {
        ...
        if (await maybeReflect('budget', brief)) {
      ...
      if (stuckStreak >= loopCfg.maxStuckRepeats) {
        // Escape hatch before aborting: give the model a moment of silence to
        // pivot. If it fires, refund the step and re-plan with the pivot in
        // history; only abort if reflection is unavailable (capped/cooldown/off).
        if (await maybeReflect('stuck', brief)) {
          iter--;
          continue;
        }
      ...
      if (emptyPlanStreak >= loopCfg.maxEmptyPlans) {
        ...
        if (await maybeReflect('empty-plan', brief)) {
          iter--;
          continue;
        }
      ...
      for (let i = 0; i < observations.length; i++) {
        ...
        events.push(entry);
        const scrollWarning = scrollLoopWarning(step, scrollLoopState, loopCfg);
