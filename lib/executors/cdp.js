...
const { navigate, ensureInView } = require('./page');
...
async function click({ session, brief, ref }) {
  ...
  // wants coordinates relative to the viewport. Scroll the target in if its
  // center is off-viewport, then subtract the LIVE scroll offset that ensureInView
  // reports (the brief's offset is stale once we've scrolled).
