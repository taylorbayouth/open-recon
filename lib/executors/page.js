...
async function pickHittablePoint({ client, backendNodeId, ref, rects }) {
  ...
  if (typeof client.DOM?.getNodeForLocation !== 'function') return center;   // fail open
  ...
  for (const p of points) {
    ...
    if (verdict === false) { sawVerdict = true; if (coveredById == null) coveredById = hitId; }
  }
  if (!sawVerdict) return center;   // machinery never decided → fail open

  const label = coveredById != null ? await describeNodeForError(client, coveredById) : 'another element';
  throw new Error(
    `click target ${ref} is covered at (${Math.round(center.x)},${Math.round(center.y)}) by ${label} — ` +
