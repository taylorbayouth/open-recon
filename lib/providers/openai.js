...
async function plan(req) {
  ...
  const body = {
    model,
    // gpt-5.4-mini is a modern agentic model: it rejects a non-default
    // `temperature` (hence omitted) and the legacy `max_tokens` field on
    // chat/completions, requiring `max_completion_tokens` instead. If you point
