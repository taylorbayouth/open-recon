...
//
// Exception: @t (StaticText/text-role) refs already have their full text in
// node.name from the AX tree. Dragging for those causes visible cursor glitch
// and often returns empty because sites use user-select:none. Skip the drag
// and return node.name directly.
async function selectText({ session, brief, ref, client, humanize, save }) {
  ...
  if (ref.startsWith('@t') && typeof node.name === 'string' && node.name.trim()) {
