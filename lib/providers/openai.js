...
async function plan(req) {
  ...
  // Reasoning effort (gpt-5 / o-series): minimal|low|medium|high. More
  // deliberation per turn helps the model track its own progress and converge —
  // e.g. notice it has already visited a page and should finish rather than loop.
  // A stable per-run field, kept near the front for the cache prefix. Only sent
  // when set, so a non-reasoning model (reasoningEffort:null) isn't handed a
