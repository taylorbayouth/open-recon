...
// lived `browser-input` Swift helper. CGEvents are posted at the HID layer, so
...
// Requires:
//   - macOS
//   - The `browser-input` binary built once via native/macos/browser-input/build.sh
...
async function selectText({ session, brief, ref, client, humanize }) {
  ...
  const start = await pageToScreen(session, bbox.x + 1, bbox.y + 1, { inputClient: client });
  const end = await pageToScreen(session, bbox.x + bbox.width - 1, bbox.y + bbox.height - 1, { inputClient: client });
