...
// Returns the debug port open-recon should actually connect to. Normally the
...
async function ensureChrome(port) {
  if (await isRunning(port)) {
    ...
    process.stderr.write(
      `[preflight] WARNING: port ${port} is in use by a browser/process open-recon did not launch.\n` +
      `  Not attaching to it (it could be your personal Chrome). Starting an isolated\n` +
      `  open-recon Chrome on port ${freePort} instead.\n`
    );
