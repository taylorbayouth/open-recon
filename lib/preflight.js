  ...
  return new Promise((resolve) => {
    ...
    child.stdout.on('data', d => {
      ...
      try {
        ...
        finish(resp.ok === true && resp.data?.trusted === true);
      } catch { finish(false); }
...
async function ensureCreds(provider) {
  ...
  if (process.env[keyName]) {
    // Warn if shell env is shadowing a different value in .env — this is how
    // wolfpack's stale key kept winning even after .env was updated.
