...
function normalizeRef(ref) {
  return typeof ref === 'string' && /^[et]\d+$/.test(ref) ? '@' + ref : ref;
}
...
function validate(actions, lookup, registry) {
  ...
  for (const action of actions) {
    ...
    if (spec.requiresRef) {
      ...
      if (!ref) { errors.push({ action, error: `verb "${verb}" requires a ref` }); continue; }
      if (!REF_RE.test(ref)) { errors.push({ action, error: `ref "${ref}" does not match /^@[et]\\d+$/` }); continue; }
      const refType = ref[1];
