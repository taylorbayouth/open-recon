...
const fs = require('fs');
const os = require('os');
const { ensureInView, normalizeUrl, waitForLoad } = require('./page');
...
async function navigate({ url, client, humanize, session }) {
  const u = normalizeUrl(url);
  await ensureInputSafe(client);
  await client.send({ op: 'key', key: 'l', modifiers: ['cmd'] });
  // Cmd+L focuses the omnibox and selects its contents, but focus doesn't land
  // instantly — typing immediately drops the first character (e.g. "https" →
  // "ttps", which Chrome then treats as a search query). Let focus settle first.
  await sleep(200);
  // Type the URL into the omnibox (browser chrome — the page can't observe it, so
  // no stealth need for humanized timing). But NOT instant: a 0ms gap between keys
  // raced the Cmd+L modifier release, so a "t" in "http(s)" could land as Cmd+T
  // and open a new tab. A small fixed delay lets the modifier/event queue settle —
  // still ~3x faster than humanized page typing, with none of the glitching.
  await client.send({ op: 'type', text: u, delayMsMin: 12, delayMsMax: 25 });
  await ensureInputSafe(client);
  // Arm the load wait (over CDP) before the Enter keystroke that triggers the
  // navigation, so a fast load can't fire before we're listening. The OS path
  // drives the keyboard; readiness is still observed through the CDP session.
  let loaded = null;
  if (session?.client) {
    await session.client.Page.enable();
    loaded = waitForLoad(session.client);
  }
