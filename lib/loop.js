...
async function run({ session, task, config = {}, verbose = false } = {}) {
  ...
  try {
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      if (verbose) {
        const cached = completion.usage?.cacheReadTokens;
        const total = completion.usage?.inputTokens;
        const cacheNote = cached ? ` [cache: ${cached}/${total} tokens (${Math.round(cached/total*100)}%)]` : '';
        console.error(`[loop] LLM planned ${plannedActions.length} action(s): ` +
          plannedActions.map(a => describeAction(a, brief)).join('; ') + cacheNote);
        if (!plannedActions.length && modelText) {
          console.error(`[loop] no action — model said: ${modelText}`);
        }
      }
...
function finish(runArtifact, { events = [], scratchpad, verbose = false } = {}) {
  ...
  if (verbose) {
    ...
    console.error(`[loop] finished with status ${runArtifact.status}; steps=${s.stepCount}; estimated prompt tokens=${s.totalEstimatedPromptTokens}; provider input/output tokens=${s.totalInputTokens}/${s.totalOutputTokens}${errNote}`);
  }
  return runArtifact;
}
