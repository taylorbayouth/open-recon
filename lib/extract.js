...
function buildTreeInteractive(node, props, bbox, inViewport) {
  const out = { role: node.role?.value };
...
function buildTreeText(node, props, bbox, inViewport) {
  const out = { role: node.role?.value, name: node.name.value };
...
function buildTree(axResult, maps, viewport, opts) {
  ...
  function collect(nodeId) {
    ...
    if (!node.ignored) {
      ...
      if (hasRole || isCustomWidget) {
        ...
        counts.interactive++;
        return [buildTreeInteractive(node, props, bbox, inViewport)];
      }
      ...
      if (TEXT_ROLES.has(role) && node.name?.value?.trim()) {
        ...
        if (opts.inViewportOnly && !inViewport) return [];
        counts.text++;
        return [buildTreeText(node, props, bbox, inViewport)];
      }
    }
  ...
  if (!roots.length) return { tree: null, counts };
  const root = roots.reduce((a, b) =>
    (b.childIds?.length || 0) > (a.childIds?.length || 0) ? b : a
  );
  const result = collect(root.nodeId);
  return { tree: result[0] || null, counts };
}

// ─── Flat mode builders ───────────────────────────────────────────────────────

function buildLeanElement(node, props, backendNodeId, bbox, inViewport) {
  const out = { role: node.role?.value ?? null, name: node.name?.value ?? null, backendNodeId, bbox, inViewport };
  const value = node.value?.value;
  if (value != null && value !== '') out.value = value;
  if (props.url) out.url = props.url;
  if (props.checked != null) out.checked = props.checked;
...
async function performExtract(client, opts = {}) {
  ...
  if (format === 'tree') {
    const { tree, counts } = buildTree(axResult, maps, viewport, opts);
    return {
      ...base,
      tree,
      stats: {
  ...
  for (const { node, props, source } of interactiveNodes) {
    ...
    if (format === 'lean') {
      elements.push(buildLeanElement(node, props, id, bbox, inViewport));
    } else {
      elements.push({
  ...
  for (const { node, props } of textNodes) {
    ...
    if (format === 'lean') {
      const t = { role: node.role?.value ?? null, name: node.name.value, bbox, inViewport };
  ...
  return { ...base, elements, text, stats };
}
