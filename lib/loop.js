  ...
  try {
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      const completion = await plan(
        // cacheKey groups this run's turns for OpenAI prompt caching (ignored by
        // other providers, which cache via explicit breakpoints). Stable for the
        // whole run, so every turn routes to the same warmed prefix.
