...
function collectRegions(snapshot, maps, viewport, opts) {
  ...
  for (const doc of snapshot.documents || []) {
    ...
    for (let i = 0; i < nodeName.length; i++) {
      ...
      regions.push({ role: REGION_ROLE[tag], bbox, inViewport });
    }
  }
  return regions;
}
...
async function performExtract(client, opts = {}) {
  ...
  // Rendered, unnamed graphics the listing above can't speak to. Surfaced so
  // the planner can take_screenshot to read them (charts, maps, scanned text,
  // CAPTCHAs). Not added to `lookup`: no verb targets a region by ref — they're
  // perception context, and take_screenshot captures the viewport, not a ref.
