...
async function performExtract(client, opts = {}) {
  ...
  // Snapshot bounds are in device pixels; derive the device→CSS scale so
  // getBounds can normalize them. The visual-viewport pair is the most direct
  // signal: non-css clientWidth is device px, css clientWidth is CSS px, so
  // their ratio is the device pixel ratio. Fall back to the layout-viewport
  // pair, then to 1 (1:1 display) if neither is reported.
  const vv = layoutMetrics.visualViewport, cssVv = layoutMetrics.cssVisualViewport;
  const lv = layoutMetrics.layoutViewport, cssLv = layoutMetrics.cssLayoutViewport;
  const ratio =
    (vv?.clientWidth && cssVv?.clientWidth) ? vv.clientWidth / cssVv.clientWidth :
    (lv?.clientWidth && cssLv?.clientWidth) ? lv.clientWidth / cssLv.clientWidth :
    1;
  maps.scale = Number.isFinite(ratio) && ratio > 0 ? ratio : 1;
