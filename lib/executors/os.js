...
async function ensureFrontmost(client, opts = {}) {
  ...
  for (;;) {
    ...
    if (CHROME_BUNDLES.has(info?.bundleId)) {
      if (announced && verbose) console.error('[os] Chrome is frontmost again; resuming.');
      return;
    }
...
function create(opts = {}) {
  ...
  return {
    ...
    async waitUntilReady() { await ensureFrontmost(client); },
