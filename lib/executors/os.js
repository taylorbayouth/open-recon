...
async function navigate({ url, client, humanize, session }) {
  ...
  await sleep(200);
  // Type the URL instantly: it goes into the omnibox (browser chrome), which the
  // page can't observe, so humanized timing here buys no stealth — only latency.
  // (Humanized timing still applies to the `type` verb, which targets page fields.)
  await client.send({ op: 'type', text: u, delayMsMin: 0, delayMsMax: 0 });
  await ensureInputSafe(client);
