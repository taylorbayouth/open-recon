...
function describeAction(action, brief) {
  ...
  switch (action.verb) {
    ...
    case 'take_screenshot': return action.args?.hint ? `looked at the page (${action.args.hint})` : 'looked at the page';
    case 'save_text': return 'saved text';
    case 'save_file': return 'saved file';
    case 'get_images': return 'listed page images';
    case 'get_files': return 'listed page files';
...
function actionKey(brief, action) {
  ...
  const node = [...(brief.elements || []), ...(brief.text || []), ...(brief.regions || [])].find(n => n.ref === action.ref);
...
// Format a token count with optional cache note: "1,820 in · 340 out" or
// "[cache 1,200/1,900 63%]  1,900 in · 340 out".
function fmtTokens(usage) {
  if (!usage) return '';
  const fmt = (n) => (n || 0).toLocaleString();
  const cached = usage.cacheReadTokens;
  const total = usage.inputTokens;
  const cacheNote = (cached && total)
    ? `[cache ${fmt(cached)}/${fmt(total)} ${Math.round(cached / total * 100)}%]  `
    : '';
  return `${cacheNote}${fmt(total)} in · ${fmt(usage.outputTokens)} out`;
}
...
async function run({ session, task, config = {}, verbose = false } = {}) {
  ...
  // Single output channel — all terminal lines go through here so formatting
  // stays consistent and toggling verbose is one switch.
  const emit = verbose ? (msg) => console.error(msg) : () => {};
  ...
  try {
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      const curUrl = cleanUrl(brief.url);
      if (curUrl && curUrl !== cleanUrl(prevUrl)) {
      ...
      const tokStr = fmtTokens(completion.usage);
      emit(`\nturn ${iter}/${loopCfg.maxSteps}${tokStr ? `  ${tokStr}` : ''}`);
      if (plannedActions.length) {
        emit(`  → ${plannedActions.map(a => describeAction(a, brief)).join('; ')}`);
      } else if (modelText) {
      ...
      //  - an `idempotentRead` verb (take_screenshot, get_images, get_files)
      //    repeated on a page that hashes identical to the one it last ran on.
      //    These read the page and produce the same output every time on an
      //    unchanged page regardless of args, and they null lastHash (so `changed`
      //    is always true and the first test can't catch them). Accumulators
      //    (save_text/save_file) are deliberately NOT flagged: saving different
      //    items on one page is productive, not flailing.
      const sameAction = !changed && lastActionKey && primaryKey && primaryKey === lastActionKey;
      const repeatedRead = primary && primary.verb === lastActionVerb
        && actions[primary.verb]?.idempotentRead === true
      ...
      for (let i = 0; i < observations.length; i++) {
        ...
        if (step.action.verb === 'done') {
          ...
          emit(`  ✓ done${step.action.args?.result ? ` — ${step.action.args.result}` : ''}`);
          return finish(runArtifact, { events, scratchpad, verbose });
        }
        ...
        if (failed) {
          ...
          // and a downstream step (e.g. composing an email about the image) needs
          // the full details. Kept untruncated; the vision call's prompt +
          // maxTokens already bound its size.
        ...
        events.push(entry);
        // Suppress the vision description in terminal output — it's in the event
        // log for the model but too verbose for the operator reading live output.
        const termLine = step.action.verb === 'take_screenshot' ? line : entry;
        emit(failed ? `  ✗ ${termLine}` : `  ✓ ${termLine}`);
      }
