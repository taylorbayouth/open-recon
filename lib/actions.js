...
const ACTIONS = {
  ...
  screenshot: {
    ...
    changesPage: false,
    description: 'Capture the visible page as an image: it is SAVED to the run\'s folder AND described by a vision model. Use it (a) when the text listing can\'t tell you what you need — an image CAPTCHA, a chart, a photo, a canvas — or (b) when the task asks you to take/save a screenshot. The result reports the saved file path and the description. One capture is enough: read the description, then finish with `done` and put the saved path in your result — do NOT repeat screenshot to "provide" the image (the file already exists). Optional `hint` names what to focus on (e.g. "read the distorted characters", "is the box checked").',
  },
  done: {
    requiresRef: false,
    args: { result: 'string?' },
