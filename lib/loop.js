...
async function run({ session, task, config = {} } = {}) {
  ...
  //   if (e === 'aborted')   return finish(runArtifact, { events, scratchpad });
  ...
  try {
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      if (stuckStreak >= loopCfg.maxStuckRepeats) {
        // Escape hatch before aborting: a moment of silence to pivot. If it fires,
      ...
      // lost, not reading. Give it a moment of silence to pivot. Unlike the stuck
