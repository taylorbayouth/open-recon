...
// describe. It has no ref (no verb targets it) — the inline instruction points
// the model at take_screenshot, and the coords tell it where to scroll first.
function formatRegion(r, view) {
  ...
  return `[unreadable]  ${role}  ${dims}— take_screenshot to read${coordSuffix(r.bbox, view)}`;
}
