...
// Pure arithmetic half of pageToScreen — split out so it can be unit-tested
// with fixtures (the CDP round-trips that feed it can't be). Inputs:
//   windowBounds  { left, top, width, height }  — Browser.getWindowBounds (DIPs)
//   layout        { clientWidth, clientHeight, pageX, pageY }  — cssLayoutViewport
//   visual        { clientWidth, clientHeight }                — cssVisualViewport
//
// Derive the chrome offset by subtraction. Chrome doesn't expose "title bar +
// tab strip + URL bar + bookmarks bar height" as one value, but the difference
// between the OS window and the rendered viewport must be exactly that. The
// `88` fallback is a sensible default for a normal Chrome window with the
// bookmarks bar visible — it only kicks in if Chrome returns no viewport
// metrics at all.
function computeScreenPoint({ windowBounds, layout = {}, visual = {}, pageX, pageY }) {
...
async function pageToScreen(session, pageX, pageY) {
  ...
  return computeScreenPoint({ windowBounds, layout, visual, pageX, pageY });
}
...
async function scroll({ brief, direction, amount, client }) {
  const vp = brief?.viewport || {};
  const dist = Number.isFinite(amount) && amount > 0
    ? amount
    : Math.round((vp.height || 800) * 0.8);
  // CGEvent pixel scroll convention is the inverse of DOM wheel: a positive
  // wheel1 scrolls the view up (content moves down), so "down" → negative dy.
  // NOTE: this sign is unverified on hardware — confirm with a manual smoke run
  // before trusting the OS backend's scroll direction.
  const dy = direction === 'up' ? dist : -dist;
  await client.send({ op: 'scroll', dx: 0, dy });
}

async function key({ key: keyName, client }) {
  // The Swift helper validates the name and rejects unknown keys; that error
  // propagates up as the Observation's error.
  await client.send({ op: 'key', key: keyName });
}
...
function create(opts = {}) {
  ...
  return {
    ...
    scroll: (args) => scroll({ ...args, client }),
    key:    (args) => key({ ...args, client }),
    navigate,  // page-level; uses session.client (CDP), no HID involved
  };
}

module.exports = { create, DEFAULT_HUMANIZE, computeScreenPoint };
