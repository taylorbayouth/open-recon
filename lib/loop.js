...
async function run({ session, task, config = {}, verbose = false } = {}) {
  ...
  try {
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      // if it changed the page, we proceed immediately. But if every action
      // errored (nothing executed, so the page can't have changed), clear
      // lastHash so the next turn re-prompts immediately rather than polling
      // for a change that can't come — same reasoning as the validation path.
